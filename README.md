...
| **4** | | **API Testing** | |
| | | Для проведения API тестирования в Postman использовать API и документацию https://reqres.in/ Представим такой сценарий : **1)** Пользователь регистрируется на сайте, **2)** Его данные сохраняются в бэк-енд, **3)** Затем данные передаются в систему, чтобы пользователь мог сделать log in без повторной регистрации.| |
